...
struct MainMenuView: View {
    var body: some View {
        GeometryReader { geometry in 
           ...
           // SpriteKitView(scene: MenuScene(size: CGSize(width: 1024, height: 1366)), size: CGSize(width: 1024, height: 1366))
        }
        .ignoresSafeArea()
    }
}
...
class SplashScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let wait = SKAction.wait(forDuration: splashTime)
            ...
            if let name = touchedNode.name {
                ...
                case "playButton":
                    if let playButton = touchedNode as? SKSpriteNode {
                        ...
                        let gameScene = DependencyGameScene(size: self.size)
