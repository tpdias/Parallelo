        ...
        if(lvl == 0) {
            ...
            for worker in workers {
                ...
                if(selectedTool.sprite.intersects(worker.sprite)) {
                    ...
                    if(requirement.type == selectedTool.type && !worker.finishedWorking) {
                        requirement.alpha = 1
                        requirement.isMet = false
                    }
                    if(secondRequirement.type == selectedTool.type && !worker.finishedWorking) {
                        secondRequirement.isMet = false
                        secondRequirement.alpha = 1
                    }
                } 
                if(count == 0) {
