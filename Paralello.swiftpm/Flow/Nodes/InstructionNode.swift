...
class InstructionNode: SKNode {
    ...
    init(instruction: String, title: String, size: CGSize) {
        ...
        instructionLabel.fontSize = AppManager.shared.appFontSizeTexts
    ...
    //    if(instructionLabel.frame.height > 260) {
            instructionLabel.position = CGPoint(x: instructionTitle.position.x, y: instructionTitle.position.y - instructionTitle.frame.size.height * 2 - instructionLabel.frame.height)
//        } else {
...
//        }
        ...
        startButton.position = CGPoint(x: instructionLabel.position.x, y: instructionLabel.position.y - 150)
